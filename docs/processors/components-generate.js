...
module.exports = {
  ...
  process: function(docs, config, componentDocs) { 
    var options = config.get('processing.componentsGenerate', {});
    var demoOutputFolder = options.demoOutputFolder;
    var componentOutputFolder = options.outputFolder;
    ...
    _(docs)
      .groupBy('componentId')
      .each(function(docsForId, id) {
        var component = {};
        component.id = docsForId[0].componentId;
        component.name = docsForId[0].componentName;

        var byType = _.groupBy(docsForId, 'type');

        var demos = {};
        _(byType.demo)
          .groupBy('id')
          .each(function(demoDocs, demoId) {
            _.each(demoDocs, function(doc) {
              _.remove(docs, doc);
            });
            ...
            //Concat the files into componentDocs. Don't use .concat because
            //that requires us to reassign the array 
            _.each(demo.files.concat(demo.indexFile), function(file) {
              componentDocs.push(file);
            });
